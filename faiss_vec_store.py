...
class FaissVectorStore:
    def __init__(
        ...
        self.embedding_model = OpenAIEmbeddings(model=embedding_model)
        self.load_local = load_local
        self.dimension = self._get_embedding_dim()
        
        # Initialize FAISS index and metadata storage
        if load_local and os.path.exists(f"{index_path}.index"):
            self.load_index()
        else:
            self._create_index()
    ...
    def _create_index(self):
        ...
        self.index = faiss.IndexFlatIP(self.dimension)  # Using Inner Product (equivalent to cosine similarity)
        self.metadata = []
    ...
    def load_index(self):
        ...
        logger.info(f"Loaded index with {len(self.metadata)} vectors from {self.index_path}")
    ...
    def add_documents(self, file_paths: Union[str, List[str]]) -> int:
        ...
        texts = [chunk["text"] for chunk in chunks]
        headings = [json.dumps(chunk["headings"]) if chunk["headings"] else "[]" for chunk in chunks]
        text_embeddings = np.array(self.embedding_model.embed_documents(texts), dtype=np.float32)
        heading_embeddings = np.array(self.embedding_model.embed_documents(headings), dtype=np.float32)
        
        # Combine embeddings with weights (70% content, 30% headings)
        combined_embeddings = 0.7 * text_embeddings + 0.3 * heading_embeddings
